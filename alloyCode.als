...
// a station can't be supplied by two DSO
fact oneDSO {
	all disj d1, d2: DSO | (d1.supply & d2.supply) = none
}
...
// a Driver has a reservation but no socket has a matching code
assert reservationIsMatched {
	all r: Reservation, c :ChargingSocket | 
	( r != none and c != none ) implies c.nextCode = r.code
}

//check reservationIsMatched

// a station is supplied by two DSO
assert oneDSO {
	all d1, d2: DSO | 
	d1 != d2 implies (d1.supply & d2.supply) = none
}
//check oneDSO // usato per scoprire che mancava un fact

all s: Section | s.capacity > 1
}
run showFull for 10
*/
